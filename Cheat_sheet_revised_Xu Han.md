...
3.  **set**
   ...
   - 生成集合：`s=set()`，`s={1,2,3}`
   - `s.add(x)`,`s.remove(x)`，后者如果x不存在会报错
   - 集合中判断in/not in是O(1)
...
5. **char**
   ...
   - `ord(a)`，返回97（注：a是97，A是65）
   - `chr(97)`，返回'a'
   - `c.isdigit()`, `c.isupper()`, `c.islower()`, `c.isalpha()`

6. **string**

   - `s.find(sub)`，寻找子串，找不到返回-1
   - `s.replace(old,new)`，把old子串替换成new
   - `s.split(sep=',')`
   - `s.strip(chars='\n')`，移除头尾的指定字符
   - `s.join(map(str,lst))`
...
9. **math**
   ...
   - `import math`
   - `ceil(), floor(), abs(), pow(x,y) #x的y次方, sqrt(), log(x, base), gcd(a,b), lcm(a,b)`
...
11. **interable**
    ...
    - `it=iter(a)`，从a生成迭代器；
    - `x=next(it)`，反复调用next可以遍历it，`next(it,default)`可以设置遍历完成后的默认返回值。
...
- 最大上升子序列：`dp[i]`是以`A[i]`结尾的LIS的个数（bisect的用法）。
...
- 最长公共子串：将两个字符串放在一块对比，`dp[i][j]`代表string1的第i个和string2的第j个，状态转移方程为`dp[i][j]=dp[i-1][j-1]+1 if str1[i-1]==str2[i-1]`。
- 最长公共子序列：允许不连续，所以需要记录上一次连续的末尾值。状态转移方程为，在最长公共子串的基础上添加`else: dp[i][j]=max(dp[i-1][j],dp[i][j-1])。`

---

#### 1.背包问题
- 经典做法：`dp[i][j]`代表前i件物品放入容量为j的背包的最大价值，状态转移方程为`dp[i][j]=max(dp[i-1][j],dp[i-1][j-Wi]+Vi)`，其中W是重量，V是价值。

- **滚动数组**的改进做法：由于`dp[i-1]`这一行只会影响`dp[i]`这一行，所以直接用一维的`dp[i]`表示处理到当前物品时背包容量为i的最大价值。

- 注意dp数组的初始化：如果要求恰好装满则容量为0的时候初始化成0，其他为-inf；如果有可能装不满均为0。
...
def total_bag():
    dp=[0]*(V+1)
    for i in range(1,n+1):           #每个物品
        for j in range(1,cost[i]+1):       #正向遍历每个容量，这样dp[j-cost[i]]就包括已经用了若干次i的情形
            dp[j]=max(dp[j],price[i]+dp[j-cost[i]])
    return dp[-1]
```
...
def d(s): #求最长上升子链长度
    ...
    return len(lst)
```
注：`bisect_left(lst,i)`：默认lst已经排序，返回i应该插入的位置（即保证i的位置之后的数都大于等于i）；`bisect_right(lst,i)`则保证之前的数都小于等于i。比如`lst=[1,2,3,3,4,5]`，`i=3`，则left返回2，right返回4。
...
二位前缀和数组的生成：`prefix[i][j]=matrix[i-1][j-1]+prefix[i-1][j]+prefix[i][j-1]-prefix[i-1][j-1]`;
...
使用prefix和prefix_map来记录已有的前缀和，从而判断子串和为0的子串个数（注：`pre[i]==pre[j]`则说明i到j-1之间的和为0；或找相同前缀和数字出现的最远位置。
例题：
...
- 注意deque的访问（`dq[i]`）是O(n)，而list是O(1)。
...
while True:
    ...
    if s[-1].isdigit():
        ...
        print(min_so_far[-1])
```
此外，还有常用的**单调栈**(monotonic stack)，其优点是：若维护了一个单调递增栈，则每次取出栈顶元素时，**新的栈顶元素**和**不符合条件而未入栈的元素**恰好是**取出的元素两侧的距离最近的比其小的元素**；单调递减栈类似。
